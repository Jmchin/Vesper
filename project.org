...
** Proposed Implementation
   ...
   the issues we had in for our microblogging platform. As before, the
   ...
   connecting. The Leader server will then append the Entry to its Log,
   and will replicate the entry to other servers in the cluster via
   RPC. Once a majority of servers have appended the entry to their
   own logs, the leader server will mark the entry as committed, apply
   its operation to the StateMachine, and returns the result to the
   calling client.
