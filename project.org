...
** Introduction
   ...
   microblogging platform using tuplespaces. The underlying model has a
   user with an individual tuplespace that they interact directly with.
   Any changes that they make to their local tuplespace (i.e write and
   take operations), should be propagated and replicated to all other
   clients in the microblog group.

   Project two saw us implementing this model in two different ways. In
   the first approach we used a centralized architecture, where each
   user in the microblog platform had an individual tuplespace/adapter
   pair. This approach had the benefit of relatively easy discovery. As
   new tuplespaces joined the multicast group, their presence would be
   recorded by the nameserver. A command line client then connects to
   this naming server at a well-known address, retrieves a list of
   client-address bindings, and performs operations directly on the
   user's tuplespace. Assuming no failures, this is obviously a correct
   approach, such that all the operations (if they succeed), will occur
   in the same order on each tuplespace.

   The second approach had us implementing a more decentralized
   ...
   from having multiple points of failure, where a single link failing
   brings the entire system into an unusable state. The second approach
   suffers from a separate, yet ultimately fatal flaw, wherein a single
   message sent into the platform will bounce around indefinitely
   between all the tuplespaces as they multicast their events, and
   respond in turn to those events, ad infinitum.

** Flaws in the previous designs
*** Single-point of failure
    The first approach introduces singular points of failure in both
    the naming server and the recovery server. Once either of these
    systems go down, there is no way for the system to autonomously
    recover and the entire platform becomes unusable until further
    administrative action is taken to bring the system back into a
    stable state.

    We attempted to ameliorate this problem with approach two, with
    our decentralized architecture. However, we found that without
    some additional implementation that this was an untennable
    approach. At a minimum we would need to sign each message with a
    sequence number and an origin so that we can ignore messages that
...
*** Sequential Consistency
    ...
    all of the data stores. There was no consensus algorithm for
    coordinating tuplespace operations between all of the servers.
    Without a mechanism for consensus, our tuplespaces can quickly
    fall into an unsynchronized state. This issue arises from the fact
    that the original architecture used a single phase commit model,
    wherein a tuplespace would respond immediately to received events.
    In the event of any failures, whether in the tuplespace, the
    adapter, the proxy or the tuplespace manager, the consistency of
    the system is lost.
    ...
    apply the entry to their data stores.
...
** Integrating tuplespaces with Vesper library
** References
   ...
   \noindent[[https://blog.pythonisito.com/2012/08/distributed-systems-with-zeromq.html]]
